...
- `names[].O`: `magazijn-a`.
- Endpoints: `manager`, `controller`, `inway`, `txlog`. Elke csr draagt naast de lokale naam
  (`<endpoint>.magazijn-a.fsc-test.local`) de ZAD-SAN's: de externe mesh-host
  ...
  door `gen-csr.sh` uit de ZAD-topologie-env afgeleid — wijzig het project via `ZAD_PROJECT`, niet
  door de csr's met de hand te bewerken.
...
> cd pki && ./issue.sh -f && ./gen-crl.sh && ./verify.sh && ./zad-bundle.sh magazijn-a
> ```
>
...
Zie ook `pki/certportal-proof.md` voor het alternatief via `ca-cfssl`/`ca-certportal`.
...
for f in pki/peers/magazijn-a/*/csr.json; do
